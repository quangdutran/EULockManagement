...
public class GenericTimeRangeValidator implements ConstraintValidator<GenericTimeRangeValid, TimeRangeRequest> {
    @Override
    public void initialize(GenericTimeRangeValid constraintAnnotation) {
    }

    @Override
    public boolean isValid(TimeRangeRequest request, ConstraintValidatorContext constraintValidatorContext) {
        if (request == null || Objects.isNull(request.getFrom())
